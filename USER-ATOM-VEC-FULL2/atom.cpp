...
void Atom::peratom_create()
{
  ...
  add_peratom("sym_type",&sym_type,INT,0);
...
//  if (nwords > avec->size_data_atom +1) imageflag = 1;
...
void *Atom::extract(const char *name)
{
  ...
  if (strcmp(name,"sym_type") == 0) return (void *) sym_type;
...
int Atom::extract_datatype(const char *name)
{
  ...
  if (strcmp(name,"sym_type") == 0) return LAMMPS_INT;
