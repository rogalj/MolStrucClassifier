...
enum{ID,MOL,PROC,PROCP1,TYPE,TYPELABEL,ELEMENT,MASS,
     ...
     TQX,TQY,TQZ,
     COMPUTE,FIX,VARIABLE,IVEC,DVEC,IARRAY,DARRAY,SYM_TYPE};
  ...
  if (nthresh) {
    ...
    for (int ithresh = 0; ithresh < nthresh; ithresh++) {
      ...
      if (thresh_array[ithresh] == ID) {
        ...
        ptr = dchoose;
        nstride = 1;
      } else if (thresh_array[ithresh] == SYM_TYPE) { // dead code?
        int *type = atom->sym_type;
        for (i = 0; i < nlocal; i++) dchoose[i] = type[i];
        ptr = dchoose;
        nstride = 1;
...
int DumpCustom::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 0; iarg < narg; iarg++) {
    ...
    } else if (strcmp(arg[iarg],"sym_type") == 0) {
      pack_choice[iarg] = &DumpCustom::pack_sym_type;
      vtype[iarg] = Dump::INT;
...
void DumpCustom::pack_sym_type(int n)
{
  int *type = atom->sym_type;
