...
int main(int argc, char *argv[])
{
	...
	if(filetype.compare("lammps_vec") == 0 || filetype.compare("lammps_vec_tri") == 0)
	{
		...
		DAFED::test_vec(x,natoms,eletype, moltype, box,parameter,filename,boxinfo, timestep, G_ID);	//inthis function, we run symmetry function calculation
	}
	return 0;
}
...
void DAFED::test_vec(double **pos,int &natoms, int *eletype, int *moltype, double *box, DAFED::CParameter &parameter,string &filename, double * boxinfo, int &timestep, int *G_ID)
{
	...
	get_allNeighbourDistances_gen_vec_sa(atoms,molec,natoms,box,parameter);
...
//This is one of the old code that accept only two vectors. Calculates symmetry function then run NN. stand alone only
...
//calculate symf and/or NN. Have option from INPUT file. stand alone only 
...
//stand alone only. proto type of calculating neighbors 
...
/*why not molec? you can do this for sa, but not for lammpe mi is a local id. This means When you get the lammps neighbour, I get the center atom and its molecule ID(which is global.) 
So I won't be able to molec[mi].COvec in lammps code. Also, if I use sa in multiple core, I have to fix molec part back to atomsatoms[molec[mi].center].NNvec.  
...
void DAFED::get_allNeighbourDistances_gen_vec_sa(CAtom *atoms, Cmolpoint *molec, int &natoms, double *box, CParameter &parameter)
{
	...
	//Validation and assiging CO and NN vector for the own molecule.  
	for (int mi = 0;mi<molcount;mi++){				//Validation for molec and getting atoms' molvec info.
		//this used to be a fail safe program for bulk system. If you have defect, this part need to be commented out. 
		//if (atmcount[mi] != natm)	
		//{
		//	cout << "Warning: some of the atoms in the molecule are not detected!" << endl;	
		//	cout << "molec[mi].center: " << molec[mi].center << endl;
...
void DAFED::get_allNeighbourDistances_gen_vec_lmp(CAtom *atoms, Cmolpoint *molec, CNeighvariables &lmpneigh, int &natoms, double *box, CParameter &parameter)
{
	...
	//Validation and assiging CO and NN vector for the own molecule.  
	for (int mi = 0;mi<molcount;mi++){				//Validation for molec and getting atoms' molvec info.
/*commented out because if you have defect that has different atomic number, it would not work.
 		if (atmcount[mi] != natm)	
		{
			cout << "Warning: some of the atoms in the molecule are not detected!" << endl;	
			cout << "molec[mi].center: " << molec[mi].center << endl;
...
//old code that calculate the correct position of ghost atoms 
/*
